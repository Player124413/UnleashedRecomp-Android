...
#   endif
    ...
    static const std::unordered_set<std::string> OptionalDeviceExtensions = {
        ...
        // The VkPhysicalDeviceDescriptorIndexingFeatures feature bits below are already
        // requested via pNext regardless of this extension, but on devices below API 1.2
        // (e.g. Adreno, which reports 1.1) the driver also requires the extension name itself
        // to be present in ppEnabledExtensionNames before it will accept shader modules that
        // declare the SPV_EXT_descriptor_indexing SPIR-V extension - otherwise
        // vkCreateShaderModule/vkCreateGraphicsPipelines fail validation and return
        // VK_ERROR_UNKNOWN. Without this, every bindless-descriptor shader silently fails to
        // create and the resulting NULL pipeline crashes the driver on the first draw.
...
#   endif
    ...
    uint64_t VulkanBuffer::getDeviceAddress() const {
        ...
        // The instance requests Vulkan 1.2 (where this was promoted to core), but some devices
        // (e.g. Adreno reporting apiVersion 1.1) only implement it via the KHR extension, in
        // which case volk leaves the core-named function pointer null. Fall back to the KHR name.
...
#   endif
        ...
        if (compatibleSurfaceFormats.empty() && (surfaceFormatCount > 0)) {
            // The requested format isn't supported by this surface (e.g. B8G8R8A8_UNORM isn't
            // among the RGBA-order formats some Android WSI surfaces report). Falling back to
            // whatever the surface actually supports avoids leaving pickedSurfaceFormat at
            // VK_FORMAT_UNDEFINED, which VulkanSwapChain::resize() would otherwise pass straight
            // to vkCreateSwapchainKHR with no validity check.
            fprintf(stderr, "Requested surface format %d not supported, falling back to %d.\n", requestedFormat, surfaceFormats[0].format);
    ...
    void VulkanCommandList::setPipeline(const RenderPipeline *pipeline) {
        ...
        case VulkanPipeline::Type::Compute: {
            ...
            if (computePipeline->vk == VK_NULL_HANDLE) {
                fprintf(stderr, "setPipeline: compute pipeline handle is NULL (pipeline creation must have failed earlier), skipping bind.\n");
                break;
            }
        ...
        case VulkanPipeline::Type::Graphics: {
            ...
            if (graphicsPipeline->vk == VK_NULL_HANDLE) {
                fprintf(stderr, "setPipeline: graphics pipeline handle is NULL (pipeline creation must have failed earlier), skipping bind.\n");
                break;
            }
...
#       endif
        ...
        // Only volkLoadInstance() was ever called, so device-level function pointers were
        // being resolved through the instance-level vkGetInstanceProcAddr trampoline. Per the
        // Vulkan spec this isn't guaranteed to work for device-only extension entry points
        // (e.g. VK_KHR_buffer_device_address on devices below API 1.2); some loaders (notably
        // Android's) return null for these unless volkLoadDevice() is used to resolve them via
        // vkGetDeviceProcAddr instead, which is what device-level functions should always use.
        volkLoadDevice(vk);
...
#if defined(__ANDROID__)
    // Experiment: allows loading a custom Vulkan driver (Turnip) via libadrenotools instead of
    // the vendor proprietary blob, to work around a driver-side shader compiler assertion seen
    // with this engine's buffer_device_address-based bindless resource access. Implemented in
    // UnleashedRecomp/os/android/vulkan_driver_android.cpp; falls back to nullptr (meaning: use
    // the normal loader) if no custom driver is present on the device.
...
#   ifdef VULKAN_VALIDATION_LAYER_ENABLED
        ...
        // Enable Synchronization Validation deterministically at instance-creation time (rather than
        // relying on VK_LAYER_SETTINGS_PATH/VK_LAYER_ENABLES, which left it ambiguous whether the
        // setting was actually honored by a given layer build on Android).
        ...
        validationFeatures.enabledValidationFeatureCount = uint32_t(std::size(enabledValidationFeatures));
...
#   endif
        ...
        // Optional GFXReconstruct capture. Armed at runtime by an env var (set from a marker file
        // on Android, see os/android/vulkan_driver_android.cpp) rather than a build flag, so a
        // single APK can capture on demand. All configuration goes through VK_EXT_layer_settings so
        // no adb/setprop is needed - the tester just gets a .gfxr file to send. These locals must
        // outlive vkCreateInstance, so they are declared here at function scope.
        const char *gfxreconLayerName = "VK_LAYER_LUNARG_gfxreconstruct";
        const char *gfxreconEnabledLayers[] = { gfxreconLayerName };
        std::string gfxreconCaptureFilePath;
        const char *gfxreconCaptureFileValue = nullptr;
        const char *gfxreconCompressionValue = "LZ4";
        const char *gfxreconLogLevelValue = "info";
        std::vector<VkLayerSettingEXT> gfxreconLayerSettings;
        VkLayerSettingsCreateInfoEXT gfxreconSettingsCreateInfo = {};
        if (getenv("UNLEASHED_GFXRECON_CAPTURE") != nullptr) {
            uint32_t gfxreconLayerCount = 0;
            vkEnumerateInstanceLayerProperties(&gfxreconLayerCount, nullptr);
            std::vector<VkLayerProperties> gfxreconAvailableLayers(gfxreconLayerCount);
            vkEnumerateInstanceLayerProperties(&gfxreconLayerCount, gfxreconAvailableLayers.data());

            bool gfxreconLayerFound = false;
            for (const VkLayerProperties &layerProperties : gfxreconAvailableLayers) {
                if (strcmp(layerProperties.layerName, gfxreconLayerName) == 0) {
                    gfxreconLayerFound = true;
                    break;
                }
            }

            if (gfxreconLayerFound) {
                createInfo.ppEnabledLayerNames = gfxreconEnabledLayers;
                createInfo.enabledLayerCount = 1;

                const char *envFile = getenv("UNLEASHED_GFXRECON_CAPTURE_FILE");
                gfxreconCaptureFilePath = (envFile != nullptr) ? envFile : "gfxrecon_capture.gfxr";
                gfxreconCaptureFileValue = gfxreconCaptureFilePath.c_str();

                // gfxreconstruct's setting-set name is "lunarg_gfxreconstruct"; pass the full layer
                // name too in case a build matches settings by that instead (extra entries are ignored).
                const char *gfxreconScopes[] = { "lunarg_gfxreconstruct", gfxreconLayerName };
                const struct { const char *name; const char *const *value; } gfxreconStrings[] = {
                    { "capture_file",             &gfxreconCaptureFileValue },
                    { "capture_compression_type", &gfxreconCompressionValue },
                    { "log_level",                &gfxreconLogLevelValue },
                };
                for (const char *scope : gfxreconScopes) {
                    for (const auto &s : gfxreconStrings) {
                        VkLayerSettingEXT setting = {};
                        setting.pLayerName = scope;
                        setting.pSettingName = s.name;
                        setting.type = VK_LAYER_SETTING_TYPE_STRING_EXT;
                        setting.valueCount = 1;
                        setting.pValues = s.value;
                        gfxreconLayerSettings.push_back(setting);
                    }
                }

                gfxreconSettingsCreateInfo.sType = VK_STRUCTURE_TYPE_LAYER_SETTINGS_CREATE_INFO_EXT;
                gfxreconSettingsCreateInfo.settingCount = uint32_t(gfxreconLayerSettings.size());
                gfxreconSettingsCreateInfo.pSettings = gfxreconLayerSettings.data();
                gfxreconSettingsCreateInfo.pNext = createInfo.pNext;
                createInfo.pNext = &gfxreconSettingsCreateInfo;

                fprintf(stderr, "GFXReconstruct capture ENABLED -> %s\n", gfxreconCaptureFileValue);
            }
            else {
                fprintf(stderr, "GFXReconstruct capture requested but the layer was not found.\n");
            }
        }
